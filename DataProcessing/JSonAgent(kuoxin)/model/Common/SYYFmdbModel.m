...
@implementation SYYFmdbModel


+ (void)initialize
{
    if (self != [SYYFmdbModel self]) {
        [self createTable];
    }
}
...
+ (instancetype)modelWithModel:(id )oldModel{
    SYYFmdbModel * newMusicModel = [[self alloc]init];
...
{
    //判断是否是JKBaseModel的子类
    for (SYYFmdbModel *model in array) {
        if (![model isKindOfClass:[SYYFmdbModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            SYYFmdbModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
